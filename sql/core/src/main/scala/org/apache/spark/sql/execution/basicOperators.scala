...
case class Project(projectList: Seq[NamedExpression], child: SparkPlan) extends UnaryNode {
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    child.execute().mapPartitions { iter =>
      ...
      val inputTypes = child.output.map { x => x.dataType } 
      val outRows = iter.map { row =>
        numRows += 1
        //val rowSize = row.numFields
        //val inputCols = inputTypes
        //val projections = output
        var outRow = row
        if(!org.apache.spark.sql.CustomSettings.useOpenCL)
          outRow = reusableProjection(row)
        outRow
      }
      outRows
    }
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    child.execute().mapPartitions { iter =>
      ...
      val outRows = iter.map { row =>
        numRows += 1
        //val rowSize = row.numFields
        //val unsafeProjections = unsafeProjectList
        //val inputCols = inputTypes
        //val projections = output
        var outRow = row
        if(!org.apache.spark.sql.CustomSettings.useOpenCL)
